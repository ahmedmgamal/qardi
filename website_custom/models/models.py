...
class Purchase(models.Model):
    ...
    product_brand_id = fields.Many2one(comodel_name="website.product.brand", string="Brand", required=False, )
    financial_institution = fields.Many2one(comodel_name="financial_institution", string="financial_institution", required=False, )
...
class Sale(models.Model):
    ...
    product_brand_ids = fields.Many2one(comodel_name="website.product.brand", string="Installment", required=False, )
    financial_institutions = fields.Many2one(comodel_name="financial_institution", string="financial institutions", required=False, )
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        print("D>>D", kwargs.get('product_brand_id'))
        print("D>>D", kwargs.get('financial_institution'))
        try:
            if order_line:
                if kwargs.get('product_brand_id'):
                    order_line.write({'product_brand_id': kwargs.get('product_brand_id'),'financial_institution' : kwargs.get("financial_institution")})
                    # self.product_brand_ids=order_line.write({'product_brand_id': kwargs.get('product_brand_id')})
        except:
            pass

        self.product_brand_ids=self.order_line.product_brand_id
        self.financial_institutions=self.order_line.financial_institution
    ...
    def set_progress(self):
        self.ensure_one()
        return {
            'name': _('sale_order_offer'),
            'view_mode': 'form',
            'res_model': 'sale_order_offer',
            'type': 'ir.actions.act_window',
            'view_id': self.env.ref("website_custom.sale_order_offer_form_custom_form").id,
    ...
    offer=fields.Many2one("sale_order_offer")

class CustomSale(models.Model):
    _name = 'website.product.brand'
    _description = 'CustomSale'

    name = fields.Char(string='Name',required=True, ondelete='cascade')

class create_offerr(models.Model):
    _name = 'sale_order_offer'
    rec_name="financial_id"

    name = fields.Many2one("financial_institution",string='Financial Institutions')
    value=fields.Float("Amount")
    order_id=fields.Many2one("sale.order",string="Quotations")
